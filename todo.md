...
# Sciverse v22 · 生态区块改造（Editorial 风）

- [ ] 1. 定位 EcosystemSection / 生态区块组件
- [ ] 2. 提取全站设计 token：衬线字体、主色 brand、字号刻度、间距
- [ ] 3. 重构标题：中文衬线 + 英文/品牌名 italic 衬线大字号；副标短句单行
- [ ] 4. 三列卡片：极简线性图标（无背景） + 全大写 tracking-widest 类目 + 衬线大标题 + 一句话定位 + 短分隔线 + 三条要点 + 主色文字按钮带 →
- [ ] 5. hover：卡片整体不投影，仅 → 位移 + 短分隔线延展
- [ ] 6. 间距：列间大留白（gap-x-12 ~ 16），上下 padding 加大
- [ ] 7. 数据/能力点用「· 」号短句对齐，避免装饰
- [ ] 8. 检查暗色模式与响应式（≥md 三列，<md 一列）
- [ ] 9. 保存 v22 checkpoint


---

# v23 · 生态 + 数据能力两区块官网级再设计

- [ ] 1. 生态：去除 ECOSYSTEM·003 章节标
- [ ] 2. 生态：副标删减为单句、信息密度降低
- [ ] 3. 生态：卡片图标改为更克制的衬线编号 + 主色细线大数字（01/02/03）
- [ ] 4. 生态：要点保留 2 条精华 + 1 条数字亮点（不超过 3 行）
- [ ] 5. 数据能力全景：去除 "Updated · May 2026" 英文标
- [ ] 6. 数据能力全景：去除四宫格底部进度条与 01/02/03/04 编号
- [ ] 7. 数据能力全景：副标删减、改为单行，主标改 weight 与字号
- [ ] 8. 数据能力全景：四宫格用 hairline 分隔，强化数字字号与单位排版
- [ ] 9. 数据能力全景：删除下方"原生/最新/最全"小卡（信息冗余）或合并为一行 caption
- [ ] 10. 保存 v23 checkpoint
